...
//    @Autowired
    private MsgQueue<Orders> ordersMQ;
    ...
    @Secured("ROLE_GOODS_BUY")
    @GetMapping("/buyMQ")
    public Result orderIn(Integer id, HttpSession session) throws ParseException {
        //数据验证
        Integer n = 1;
        // 获取要购买的货物信息
        Goods goods = goodsService.getById(id);
...
//    @Secured("ROLE_ORDER_SHOW")
    ...
    public Result orderlist(@RequestBody Map<String, String> map, HttpSession session) {
        ...
        QueryWrapper qe = new QueryWrapper();
        if(user.getRoleId() == 2)
            qe.like(isUId,"user_id", search_content);
        else qe.eq("user_id", user.getUserId());
        qe.like(isOId,"order_id", search_content);
        qe.like(isGId,"good_id", search_content);
